...
class FitnessMLR:
    ...
    #------------------------------------------------------------------------------
    @staticmethod
    def validate_model(write, model, fileW, population, TrainX, TrainY, ValidateX, ValidateY, TestX, TestY):
        ...
        trackR2PredValidation, trackR2PredTest  = FitnessMLR.InitializeTracks()

        yTrain, yHatTrain, yHatCV, yValidation, \
        yHatValidation, yTest, yHatTest = FitnessMLR.initializeYDimension()

        unfit = 1000
        itFits = 1
        for i in range(numOfPop):
            xi = FitnessMLR.OnlySelectTheOnesColumns(population[i])
            ...
            Yhat_cv = FitnessMLR.cv_predict(model, X_train_masked, TrainY)    # Cross Validation
            ...
            q2_loo = FitnessMLR.r2(TrainY, Yhat_cv)
            q2_loo = FromDataFileMLR.DataMLR.getTwoDecPoint(q2_loo)

            r2pred_validation = FitnessMLR.r2Pred(TrainY, ValidateY, Yhat_validation)
            r2pred_validation = FromDataFileMLR.DataMLR.getTwoDecPoint(r2pred_validation)

            r2pred_test = FitnessMLR.r2Pred(TrainY, TestY, Yhat_test)
            r2pred_test = FromDataFileMLR.DataMLR.getTwoDecPoint(r2pred_test)

            Y_fitness = append(TrainY, ValidateY)
            Yhat_fitness = append(Yhat_cv, Yhat_validation)

            fitness[i] = FitnessMLR.calc_fitness(xi, Y_fitness, Yhat_fitness, c)
            ...
            r2_train = FitnessMLR.r2(TrainY, Yhat_train)
            ...
            trackFitness[idx] = FromDataFileMLR.DataMLR.getTwoDecPoint(fitness[i])

            trackModel[idx] = model_desc

            trackR2[idx] = FromDataFileMLR.DataMLR.getTwoDecPoint(r2_train)
            trackQ2[idx] = FromDataFileMLR.DataMLR.getTwoDecPoint(q2_loo)
            trackR2PredValidation[idx] = FromDataFileMLR.DataMLR.getTwoDecPoint(r2pred_validation)
            trackR2PredTest[idx] = FromDataFileMLR.DataMLR.getTwoDecPoint(r2pred_test)

            yTrain[idx] = TrainY.tolist()

            yHatTrain[idx] = Yhat_train.tolist()
            for i in range(len(yHatTrain[idx])):
                yHatTrain[idx][i] = FromDataFileMLR.DataMLR.getTwoDecPoint(yHatTrain[idx][i])

            yHatCV[idx] = Yhat_cv.tolist()
            for i in range(len(yHatCV[idx])):
                yHatCV[idx][i] = FromDataFileMLR.DataMLR.getTwoDecPoint(yHatCV[idx][i])

            yValidation[idx] = ValidateY.tolist()

            yHatValidation[idx] = Yhat_validation.tolist()
            for i in range(len(yHatValidation[idx])):
                yHatValidation[idx][i] = FromDataFileMLR.DataMLR.getTwoDecPoint(yHatValidation[idx][i])

            yTest[idx] = TestY.tolist()

            yHatTest[idx] = Yhat_test.tolist()
            for i in range(len(yHatTest[idx])):
                yHatTest[idx][i] = FromDataFileMLR.DataMLR.getTwoDecPoint(yHatTest[idx][i])

        FitnessMLR.write(model,fileW, trackDesc, trackFitness, trackModel, trackR2,\
